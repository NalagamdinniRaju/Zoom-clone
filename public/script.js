...
showChat.addEventListener("click", () => {
  ...
  if (mainRight.style.display === "none" || mainRight.style.display === "") {
    ...
    mainRight.style.flex = "0 0 300px";
  } else {
...
socket.on("createMessage", (message, userName) => {
  messages.innerHTML +=
    `<div class="message ${userName === user ? "sent" : ""}">
        <b>${userName === user ? "You" : userName}</b>
        <span>${message}</span>
    </div>`;
...
// const addVideoStream = (video, stream, isAdmin = false) => {
//   video.srcObject = stream;
//   video.addEventListener("loadedmetadata", () => {
//     video.play();
//   });
//   if (isAdmin) {
//     video.classList.add("admin-video");
//   }
//   videoGrid.append(video);
// };
